...
\end{myvhdl}
    ...
    \begin{column}{0.5\textwidth}
      ...
      implementato utilizzando il costrutto \lstinline{with}:
      \begin{myvhdl}{datapath.vhd}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item l'ALU con un \lstinline{component} behavioral;
...
\begin{enumerate}
  ...
  \lstinline{main} che incrementa il \lstinline{PC}, esegue il fetch di una
  microistruzione e imposta come prossima microistruzione quella ``puntata'' dal
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \end{itemize}
  \end{itemize}
  \item Per lanciare la batteria di test va lanciato il target \lstinline{make
    check}
  \item Le forme d'onda (visualizzabili con \lstinline{gtkwave}) sono salvate
  nella directory \lstinline{build/src/test/vhdl}.
\end{enumerate}
Per aggiungere altri testbench alla batteria:
\begin{enumerate}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \end{itemize}
    \item Eseguire l'override di una istruzione.
    \begin{itemize}
      \item È necessario modificare il microprogramma (\lstinline{ajvm.mal}).
